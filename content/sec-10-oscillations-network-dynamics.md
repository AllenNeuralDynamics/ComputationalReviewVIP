...
The second thread is inter-areal. {cite:t}`Bastos2023` show that top-down projections from anterior cingulate to V1 synchronise in the theta/alpha band (~10 Hz) and preferentially activate VIP interneurons, which in turn disinhibit pyramidal cells via suppression of SST interneurons; silencing VIP disrupts both the inter-areal synchrony and the deviance-detection signal it supports. {cite:t}`Chalkiadakis2025` reports developmental changes in beta and gamma spectral peaks across the first postnatal weeks in cortex that co-occur with maturation of VIP circuitry, and {cite:t}`Ikebara2025` similarly links beta-band trajectory to developmental VIP integration. {cite:t}`Haziza2025` provides more recent evidence for a selective role of VIP cells in shaping beta-range temporal structure during exploratory behaviour.
